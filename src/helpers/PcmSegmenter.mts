...
const OUT_DIR = path.join(__dirname, '..', 'audio_data');
...
function frameDbfs(bufLE16: Int16Array): number {
    let sumSq = 0;
...
function pcmToWavBuffer(pcmBuf: Buffer, { sampleRate, channels, bitsPerSample }: WavOptions): Buffer {
...
export class PcmSegmenter extends EventEmitter {
    ...
    private save_segment(pcmBuf: Buffer): void {
        ...
        const filename = `tx_${Date.now()}_${Math.random().toString(36).slice(2, 7)}.wav`;
        const fp = path.join(OUT_DIR, filename);
        fs.writeFileSync(fp, wav);

        this.emit('segment', { filename });
    }
}
