...
- **Provider seam:** `src/llm/voice-provider.mts` defines `IVoiceProvider` (the contract the device consumes) and `voice-provider-factory.mts` constructs the provider selected by the `voice_provider` global setting: `openai-realtime`, `gemini-realtime` (`providers/gemini-live-provider.mts`), or `local` (`providers/local-pipeline-provider.mts`). The local provider chains on-device energy VAD (`providers/local/simple-vad.mts` — there is no server VAD locally) → Whisper STT → Ollama LLM (streaming `/api/chat` + the ToolManager tool loop) → Piper TTS, all plain-HTTP clients in `providers/local/`, configured by the `local_stt_host`/`local_llm_host`/`local_tts_host` (+ port/model) global settings. It is keyless (`hasApiKey()` is always true) and speaks replies sentence-by-sentence while the LLM streams.
